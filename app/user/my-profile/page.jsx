...
const page = () => {
  ...
  const getIlceList = async () => {
    try {
      const response = await axios.get(
        `https://api.turkiyeapi.dev/v1/districts?province=${sehir}`,
      );
      setIlceList(response.data.data);
    } catch (error) {
      console.log("error", error);
    }
  };
  ...
  const fullAddress = `${mahalle || ""}, ${ilce || ""}, ${sehir || ""}, Türkiye`;
  const googleMapsUrl = `https://maps.google.com/maps?q=${encodeURIComponent(fullAddress)}&output=embed`;
            ...
            <div className="w-full">
              ...
              <div className="border border-gray-400 bg-green-100 p-5 w-full rounded-2xl mt-5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-3">
                    <img className="w-12" src="/master.png" alt="MasterCard" />
                    <img className="w-12" src="/visa.webp" alt="Visa" />
                    <img className="w-12" src="/troy.png" alt="Troy" />
                    <img className="w-20" src="/world.png" alt="World" />
                  </div>
                </div>
  ...
  if (loading)
    return (
      <div className="w-full h-full flex justify-center items-center">
        <img className="w-32 h-32 animate-spin" src="/rim-loading.svg" alt="Loading" />
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        ...
        {user?.role === "kurumsal" && (
          <button
            ...
            onClick={() => handleClickOpen()}
            className={`px-6 py-2 bg-orange-400 text-white text-sm rounded-full ${
              hasFeaturedRequest ? "cursor-not-allowed opacity-50" : "cursor-pointer"
            }`}
          >
            Öne Çıkar
          </button>
      ...
      {user?.role === "kurumsal" && (
        <div className="w-full grid grid-cols-2 gap-x-10 mt-5">
          <div>
            ...
            />
            {logoFile && (
              <img className="mt-3 max-h-40 object-contain" src={URL.createObjectURL(logoFile)} alt="Yeni Logo Önizleme" />
            )}
            {!logoFile && logo && <img className="mt-3 max-h-40 object-contain" src={logo} alt="Mevcut Logo" />}
          </div>
          <div>
            <label className="text-sm font-semibold">Kapak Fotoğrafı</label>
            <input
            ...
            {!coverFile && cover && (
              <div className="w-full h-44 border border-gray-400 mt-3">
                <img className=" w-full h-full object-contain" src={cover} alt="Mevcut Kapak" />
              </div>
            )}
          </div>
        </div>
